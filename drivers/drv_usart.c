...
#define UART_IS_RX        (0U)
   ...
   for (index = 0; index < sizeof(uart_afs) / sizeof(struct gpio_uart_af); index = index + 1)
   {
   ...
   for (index = 0; index < sizeof(uart_afs) / sizeof(struct gpio_uart_af); index = index + 1)
   {
       if (uart_afs[index].pin_index == rx_pin_num)
       {
           if(uart_afs[index].afs[0].uart_num == (uart_num|UART_IS_RX))
           {
               rx_af_num = uart_afs[index].afs[0].af_num;
           }
       }
   }
